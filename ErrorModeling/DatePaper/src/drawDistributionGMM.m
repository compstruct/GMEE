...
%amin edit
showDashed = strcmp( style1 , '--'); % age dash bashe mishe 1  age na ke solid mizare mire dge


if ( d == 1 )
    if ( ~isempty(color) )
        showDecomposedPdf( pdf, 'linTypeSum', color, 'decompose', decompose, 'draw_to_these_axes', draw_to_these_axes, 'showDashed',showDashed ) ;
    else
        showDecomposedPdf( pdf, 'decompose', decompose, 'draw_to_these_axes', draw_to_these_axes,  'showDashed',showDashed ) ;
    end
