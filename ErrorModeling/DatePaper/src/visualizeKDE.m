...
if showTabulated ~= 1
    drawDistributionGMM( 'pdf',kde.pdf, 'color', color, 'decompose', decompose, ...
           ...
           'useEdgeColorBlack', useEdgeColorBlack, 'draw_to_these_axes', draw_to_these_axes, 'Style', style1) ;            
else
...
if ~isempty(data) && showdata == 1
    ...
    if size(kde.pdf.Mu,1) == 1
        plot(draw_to_these_axes, obsAll(1,:),obsAll(1,:)*0,'LineStyle','.r') ;
    elseif size(kde.pdf.Mu,1) == 2
        plot(draw_to_these_axes, obsAll(1,:),obsAll(2,:),'LineStyle','.r') ;
    elseif size(kde.pdf.Mu,1) == 3
        plot3(draw_to_these_axes, obsAll(1,:),obsAll(2,:),obsAll(3,:),'LineStyle','--','.r') ;
    end
    if ( h == 0 ) hold off ; end
end