...
fprintf(fileID2,'s\ns** [EM_in=%12d]\n',SigmaToEMin_MED_matrix(Sig_a,Sig_b)); % ba ina sakhtim vali badia darumade
...
errrand_A =round( normrnd(0, 2^(randi(input_err)) , [Nonums 1]));
errrand_B =round( normrnd(0, 2^(randi(input_err)) , [Nonums 1]));
...
corr_out_datas = horzcat(corr_out_datas,out_datas(:,4));% data khoruji doroste bedune khataye input
 ...
 %Sigma prints
fprintf(fileID2,'p%12d %12d\n',2^Sig_a,2^Sig_b); % ba ina sakhtim vali badia darumade
fprintf(fileID2,'s\ns** [EM_in=%12d]\n',SigmaToEMin_MED_matrix(Sig_a,Sig_b)); 
