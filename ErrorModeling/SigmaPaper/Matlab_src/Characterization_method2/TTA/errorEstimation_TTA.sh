...
${CMC_DIR}/tools/altera/15.1/modelsim_ae/bin/vlog {CLA4.v,CGEN.v,RCA.v,H_FA.v,ETA.v,TTA.v,ETAIIM.v,tb_matlab_TTA.v} | grep -E $filter| grep -v $no # filan faghat in dotaro gozashtam
${CMC_DIR}/tools/altera/15.1/modelsim_ae/bin/vsim -c -do "run -all" 'tb_matlab_TTA' | grep -E $filter| grep -v $no
printf  "${NC}\n"
thirdTime=`date +%s`
...
cd ${HOME_DIR}/approxiSynthesys/benchmarks/custom/RCA/
${CMC_DIR}/tools/altera/15.1/modelsim_ae/bin/vlog {CLA4.v,CGEN.v,RCA.v,H_FA.v,ETA.v,TTA.v,ETAIIM.v,tb_matlab_reg_TTA.v} | grep -E $filter | grep -v $no # filan faghat in dotaro gozashtam
${CMC_DIR}/tools/altera/15.1/modelsim_ae/bin/vsim -c -do "run -all" 'tb_matlab_reg_TTA' | grep -E $filter| grep -v $no
printf  "${NC}\n"
sixthTime=`date +%s`
